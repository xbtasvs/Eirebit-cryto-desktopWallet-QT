...
*Setup instructions for a Gitian build of Eirebit Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Eirebit
...
- [Building Eirebit Core](#building-eirebit-core)
...
Clone the git repositories for eirebit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/eirebit-project/eirebit
...
in the eirebit repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Eirebit Core
----------------

To build Eirebit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the eirebit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/eirebit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/eirebit-project/eirebit
...
For example:
```bash
URL=https://github.com/thrasher-/eirebit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit eirebit=${COMMIT} --url eirebit=${URL} ../eirebit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit eirebit=${COMMIT} --url eirebit=${URL} ../eirebit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit eirebit=${COMMIT} --url eirebit=${URL} ../eirebit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the eirebit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../eirebit/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/eirebit
SIGPATH=/some/root/path/eirebit-detached-sigs

./bin/gbuild --url eirebit=${BTCPATH},signature=${SIGPATH} ../eirebit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/eirebit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/eirebit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/eirebit-osx-build.assert
```
...
[eirebit-project/gitian.sigs.ltc](https://github.com/eirebit-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
